...
def render_dashboard(user_id):
    ...
    page = st.sidebar.radio("Navigation", ["Home", "Portfolio Generator", "Market Analysis", "Notifications", "Educational Content", "Settings"])
...
def generate_buy_notifications(user_id, scored_stocks, threshold=0.7):
    """
    Create notifications for stocks with Buy_Prob above a threshold.
    """
    conn = sqlite3.connect('data/investment_platform.db')
    c = conn.cursor()
    
    for _, row in scored_stocks.iterrows():
        if row["Buy_Prob"] >= threshold:
            notif_id = str(uuid.uuid4())
            c.execute("""
                INSERT INTO notifications (id, user_id, stock_symbol, buy_probability)
                VALUES (?, ?, ?, ?)
            """, (notif_id, user_id, row["Symbol"], row["Buy_Prob"]))
    
    conn.commit()
    conn.close()

# Generate notifications for strong buy signals
generate_buy_notifications(user_id, scored_full, threshold=0.7)

def render_notifications_page(user_id):
    ...
    if not rows:
        st.info("No notifications yet. Come back later!")
        return
    
    notif_df = pd.DataFrame(rows)
    notif_df["buy_probability"] = notif_df["buy_probability"].apply(lambda x: f"{x:.2%}")
    
    st.table(notif_df[["stock_symbol", "buy_probability", "created_at"]])

def render_educational_links():
    st.markdown("<h1 class='main-header'>Educational Resources</h1>", unsafe_allow_html=True)
    
    st.markdown("<div class='card'>", unsafe_allow_html=True)
    st.write("## How to Invest in Moroccan Stocks")
    st.markdown("""
    - [📝 AMMC - Moroccan Capital Market Authority Regulations](https://www.ammc.ma/)
    - [🏦 Attijariwafa Bank Brokerage Services](https://www.attijariwafabank.com/)
    - [📈 Casablanca Stock Exchange Official Website](https://www.casablanca-bourse.com/)
    - [📚 Beginner's Guide to Investing in Morocco (Article)](https://www.boursenews.ma/)
    """, unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)

    st.markdown("<div class='card'>", unsafe_allow_html=True)
    st.write("## Taxation and Legal Aspects")
    st.markdown("""
    - [📋 Moroccan Tax Guide for Investors (2024)](https://www.tax.gov.ma/)
    - [⚖️ Financial Law Overview in Morocco](https://www.finances.gov.ma/)
    """, unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)

    st.markdown("<div class='card'>", unsafe_allow_html=True)
    st.write("## Useful Investment Tools")
    st.markdown("""
    - [💡 Moroccan Investment Center - Procedures and Help](https://www.cri.ma/)
    - [📰 Latest Financial News (Boursenews)](https://www.boursenews.ma/)
    """, unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
